...
def tilt_north(rows: list[str]) -> list[str]:
    ...
    for x in range(len(rows[0])):
        # Transpose each column as a row (rotate clockwise)
        # Add a "#" to the end to make sure the last space is processed without a special case
        column = "".join(row[x] for row in rows)[::-1] + "#"
        last_cube_index = -1
        num_rocks = 0
...
def rotate_clockwise(rows: list[str]) -> list[str]:
    return ["".join(row[x] for row in rows)[::-1] for x in range(len(rows[0]))]


def calculate_load(rows: list[str]) -> int:
    return sum(row.count("O") * (len(row) - i) for i, row in enumerate(rows))


def part1(platform: list[str]) -> int:
    return calculate_load(tilt_north(platform))
...
def part2(platform: list[str]) -> int:
    ...
    while True:
        ...
        if load not in seen:
            ...
            elif (1_000_000_000 - i) % sum(periods[periods.index(period):]) == 0:
                return load
            seen[load] = i
        i += 1
...
if __name__ == '__main__':
    text = Path("../inputs/14.txt").read_text().splitlines()
    print(part1(text))
    print(part2(text))
