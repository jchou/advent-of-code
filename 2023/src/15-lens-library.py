...
def get_hash(ascii_text: str, start: int = 0) -> int:
    for char in ascii_text.strip():
        start = ((start + ord(char)) * 17) % 256
    return start


def part1(init_sequence: list[str]) -> int:
    total = 0
    for sequence in init_sequence:
        total += get_hash(sequence)
    return total


def part2(init_sequence: list[str]) -> int:
    boxes = collections.defaultdict(list)
    for sequence in init_sequence:
        sequence = sequence.strip()
        operation = "-" if "-" in sequence else "="
        entry = (label, new_lens) = sequence.split(operation)
        ...
        if not new_lens:
            ...
            for i, lens in enumerate(boxes[box_num]):
                if lens[0] == label:
                    boxes[box_num][i] = entry
                    break
            # Add entry if it doesn't exist
            else:
                boxes[box_num].append(entry)

    return sum(
        (box_num + 1) * i * int(lens[1])
...
if __name__ == '__main__':
    text = Path("../inputs/15.txt").read_text()
    print(part1(text.split(",")))
    print(part2(text.split(",")))
